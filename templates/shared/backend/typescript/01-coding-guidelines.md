...
// 制約付きジェネリクス
function serialize<T extends Record<string, unknown>>(obj: T): string {
...
async function getUserWithProfile(id: number): Promise<UserWithProfile> {
  const [user, profile] = await Promise.all([userRepository.findById(id), profileRepository.findByUserId(id)])
