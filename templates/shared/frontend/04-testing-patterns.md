...
export const handlers = [
  ...
  rest.post('/api/users', async (req, res, ctx) => {
    ...
    if (!name || !email) {
      return res(ctx.status(400), ctx.json({ error: '名前とメールは必須です' }))
    }
  ...
  rest.get('/api/users/:id', (req, res, ctx) => {
    ...
    if (id === 'not-found') {
      return res(ctx.status(404), ctx.json({ error: 'ユーザーが見つかりません' }))
    }
...
expect.extend({
  async toHaveLoadingState(locator: Locator) {
    const isLoading =
      (await locator.getAttribute('aria-busy')) === 'true' || (await locator.getByRole('status').isVisible())

    return {
      message: () =>
        isLoading ? 'Expected element not to have loading state' : 'Expected element to have loading state',
      pass: isLoading,
    }
  },
