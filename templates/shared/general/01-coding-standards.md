 ...
 * ```
 */
async function getUserById(userId: string, includePreferences = false): Promise<User | null> {
  // 実装
}
````
...
interface APIClient {
  ...
  post<TRequest, TResponse>(endpoint: string, data: TRequest): Promise<APIResponse<TResponse>>
}
