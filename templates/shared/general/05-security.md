...
class AuthService {
  ...
  async generateTokens(user: User): Promise<{ accessToken: string; refreshToken: string }> {
    ...
    const refreshToken = jwt.sign({ userId: user.id, type: 'refresh' }, this.JWT_SECRET, {
...
class EncryptionService {
  ...
  decrypt(encryptedData: string, password: string): string {
    ...
    const tagHex = encryptedData.slice(
      (this.saltLength + this.ivLength) * 2,
      (this.saltLength + this.ivLength + this.tagLength) * 2
    )
...
class GDPRCompliance {
  ...
  private async collectUserData(userId: string): Promise<any> {
    const [profile, orders, sessions, communications] = await Promise.all([
